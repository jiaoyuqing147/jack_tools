...
# ====================
# 第一步：读取统计表，找出 train>0 且 val>0 的类别
# ====================
stats = pd.read_csv(r"class_distribution_AfterDivied.csv")
valid_classes = stats[(stats["train_count"] > 0) & (stats["val_count"] > 0)]["class_id"].tolist()

print("保留下来的类别ID:", valid_classes)
new_names = [names[i] for i in valid_classes]

# 构造旧ID→新ID的映射
id_map = {old_id: new_id for new_id, old_id in enumerate(valid_classes)}
print("类别重新映射:", id_map)