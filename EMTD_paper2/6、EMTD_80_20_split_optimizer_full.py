...
class FastDatasetSplitOptimizer:
    ...
    def load_data(self) -> None:
        ...
        # 保留 GT.csv 中原始文件名的大小写。
        # Linux 区分大小写，统一转为小写可能导致找不到实际图像。
    ...
    def save_results(self) -> None:
        ...
        # 保存图像文件名，并根据图像主文件名生成对应的 YOLO 标签名。
        # 例如 IMG_001.JPG -> IMG_001.txt。
        train_image_names = sorted(train_set)
        test_image_names = sorted(test_set)
        ...
        pd.Series(train_image_names).to_csv(
        ...
        pd.Series(test_image_names).to_csv(
        ...
        pd.Series(train_label_names).to_csv(
            self.output_dir / "train_labels.csv",
            index=False,
            header=False,
            encoding="utf-8-sig",
        )

        pd.Series(test_label_names).to_csv(
            self.output_dir / "test_labels.csv",
            index=False,
            header=False,
            encoding="utf-8-sig",
        )
        ...
        print((self.output_dir / "train_labels.csv").resolve())
        print((self.output_dir / "test_labels.csv").resolve())
